 ...
 */
class BeanParser(/*beanFactory: BeanFactory*/) extends LanguageParser[Module] {
  ...
  private lazy val moduleDef: PackratParser[Module] =
    MODULE ~> identifier ~ ("{" ~> imports) ~ definitions <~ "}" ^^ {
      case name ~ imps ~ defs =>
        Module(name, imps, defs)
    }
  ...
  private lazy val definition: PackratParser[Def] = VAL ~> ident ~ expression <~ opt(";") ^^ {Def(_, _)}
  ...
  private lazy val simpleType: PackratParser[TypeDef] =
    ident ^^ {
      case typeName => beanFactory.typeFor(typeName)
    }
  ...
  private lazy val funExprParameterWithoutTypeOrDefault: PackratParser[Parameter] =
    ident ^^ {
      case name => new Parameter(Symbol(name), None, None)
    }
  ...
  private lazy val unnamedArgument: PackratParser[Arg] =
    expression ^^ {
      case expr => new Arg(expr, None.asInstanceOf[Option[Symbol]])
    }
  ...
  private lazy val mathExpr: PackratParser[Expr] =
    term ~ rep(("+" | "-") ~ term) ^^ {
      case first ~ rest => processOperations(first, rest)
    }
